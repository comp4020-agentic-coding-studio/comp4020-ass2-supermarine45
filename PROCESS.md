...
Had to manually feed the text and think of visuals to be added manually. The prototype generated by claude is overly simplistic and not very interactive.

In particular, I asked, through the prompt, to make the content grounded in actual research paper (cite), though the topic is still a mock.

Most challenge is to make the content not 'sloppy'.

Next challenge, reviewing the ppt manually and adding visuals where appropriate.

I observe, claude struggles with complex large prompts, as such the development must be done in stages.
